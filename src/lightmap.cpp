...
bool map::build_transparency_cache( const int zlev )
{
    ...
    auto &blocked_cache = map_cache.blocked_cache;
    ...
    if( rebuild_all ) {
        ...
        std::uninitialized_fill_n( &blocked_cache[0][0],
    ...
    for( int smx = 0; smx < my_MAPSIZE; ++smx ) {
        for( int smy = 0; smy < my_MAPSIZE; ++smy ) {
            ...
            if( !rebuild_all ) {
                for( int sx = 0; sx < SEEX; ++sx ) {
                    diagonal_blocks fill = {false, false};
                    std::uninitialized_fill_n( &blocked_cache[sm_offset.x + sx][sm_offset.y], SEEY, fill );
                }
            }
                            ...
                            for( int my = -1; my <= 1; my += 2 ) {
                                for( int mx = -1; mx <= 1; mx += 2 ) {
                                    ...
                                    if( !vehicle->allowed_light( t, from_mount ) ) {
                                        if( my == -1 && mx == -1 ) {
                                            blocked_cache[from.x][from.y].nw = true;
                                        } else if( my == -1 && mx == 1 ) {
                                            blocked_cache[from.x][from.y].ne = true;
                                        } else if( my == 1 && mx == -1 ) {
                                            if( from.x == 0 || from.y == MAPSIZE_Y - 1 ) {
                                                continue;
                                            }
                                            blocked_cache[from.x - 1][from.y + 1].ne = true;
                                        } else {
                                            if( from.x == MAPSIZE_X - 1 || from.y == MAPSIZE_Y - 1 ) {
                                                continue;
                                            }
                                            blocked_cache[from.x + 1][from.y + 1].nw = true;
                                        }
                                    }
