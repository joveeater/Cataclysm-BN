...
TEST_CASE( "scent_matches_old" )
{
    ...
    get_map().ter_set( origin + tripoint_south_west, t_brick_wall );
    get_map().ter_set( origin + tripoint_west, t_brick_wall );
    get_map().ter_set( origin + tripoint_north, t_rock_wall_half );
    get_map().ter_set( origin, t_rock_wall_half );
    g->scent.reset();

    g->scent.set( origin, 1000, scenttype_id( "sc_human" ) );

    g->scent.update( origin, g->m );
    g->scent.update( origin, g->m );
    g->scent.update( origin, g->m );
    ...
    old_scent_map_update( origin, g->m, old_scent );
    old_scent_map_update( origin, g->m, old_scent );
    old_scent_map_update( origin, g->m, old_scent );
    int x = 0;
    for( auto &elem : old_scent ) {
